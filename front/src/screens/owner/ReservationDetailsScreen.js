...
const OwnerReservationDetailsScreen = ({ navigation, route }) => {const { t } = useTranslation();
      ...
      <View style={styles.container}>
        <SafeAreaView style={styles.header}>
          ...
          <View style={styles.headerCenter}>
            <Text style={styles.headerTitle}>{t("screens.owner.reservationdetailsscreen.recapitulatif")}</Text>
          </View>
          <View style={{ width: 40 }} />
        </SafeAreaView>
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <Text style={{ color: '#f6f8ff', fontSize: 16, fontWeight: '700', marginBottom: 8 }}>{t("screens.owner.reservationdetailsscreen.reservationIntrouvable")}

          </Text>
          <Text style={{ color: '#8e95bf', textAlign: 'center' }}>{t("screens.owner.reservationdetailsscreen.impossibleDafficherLesDetailsDeCetteReservation")}
  ...
  const getStatusLabel = (status) => {
    const labels = {
      ...
      refund_pending: t("screens.owner.reservationdetailsscreen.remboursementEnAttente"),
      refunded: t("screens.owner.reservationdetailsscreen.rembourse"),
      payment_pending: t("components.payment.paymentstatusdisplay.paiementEnAttente"),
      active: t("screens.owner.reservationdetailsscreen.actif"),
      finished: t("screens.owner.reservationdetailsscreen.termine"),
  ...
  const STATUS_OPTIONS = useMemo(
    () => [
    { value: 'confirmed', label: t("screens.owner.reservationdetailsscreen.confirme") },
    { value: 'reserved', label: t("screens.owner.reservationdetailsscreen.reserve") },
    { value: 'payment_pending', label: t("screens.owner.reservationdetailsscreen.enAttentePaiement") },
    { value: 'pickup_pending', label: t("screens.owner.reservationdetailsscreen.enAttentePickup") },
    { value: 'active', label: t("screens.owner.reservationdetailsscreen.actif") },
    { value: 'return_pending', label: t("screens.owner.reservationdetailsscreen.enAttenteRetour") },
    { value: 'refund_pending', label: t("screens.owner.reservationdetailsscreen.remboursementEnAttente") },
    { value: 'refunded', label: t("screens.owner.reservationdetailsscreen.rembourse") },
    { value: 'finished', label: t("screens.owner.reservationdetailsscreen.termine") },
    { value: 'cancelled', label: t("screens.owner.reservationdetailsscreen.annule") }],

    []
  );

  const handleStatusInfo = () => {
    Alert.alert(t("screens.owner.reservationdetailsscreen.statut"), getStatusLabel(status));
  };

  const updateStatus = async (nextStatus) => {
    if (!reservation?.id) return;
    if (!token) {
      Alert.alert(t("screens.owner.reservationdetailsscreen.erreur"), t("screens.owner.reservationdetailsscreen.veuillezVousReconnecter"));
      return;
    }
    try {
      setLoading(true);
  ...
  const confirmCashPayment = async () => {
    ...
    try {
      ...
      Alert.alert(t("screens.owner.reservationdetailsscreen.succes"), t("screens.owner.reservationdetailsscreen.paiementEnEspecesConfirme"));
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.vehicleInfo}>
            ...
            <View style={styles.vehicleMetaRow}>
              ...
              <View style={styles.metaItem}>
                ...
                <Text style={styles.metaText}>{seats}{t("screens.owner.reservationdetailsscreen.places")}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeaderWithButton}>
            <Text style={styles.sectionTitle}>{t("screens.owner.reservationdetailsscreen.periodeDeLocation")}</Text>
          </View>

          <View style={styles.datesContainer}>
            <View style={styles.dateBox}>
              <Text style={styles.dateBoxLabel}>{t("screens.owner.reservationdetailsscreen.depart")}</Text>
            ...
            <View style={styles.dateSeparator}>
              ...
              <Text style={styles.dateSeparatorDays}>{totalDays}{t("screens.owner.reservationdetailsscreen.j")}</Text>
              <View style={styles.dateSeparatorLine} />
            </View>

            <View style={styles.dateBox}>
              <Text style={styles.dateBoxLabel}>{t("screens.owner.reservationdetailsscreen.retour")}</Text>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeaderWithButton}>
            <Text style={styles.sectionTitle}>{t("screens.owner.reservationdetailsscreen.informationsClient")}</Text>
            <TouchableOpacity
              onPress={() => {
                const otherUserId = reservation?.renter?.id || reservation?.renter_id;
                if (!otherUserId) return;
          ...
          <Text style={styles.chatHint}>{t("screens.owner.reservationdetailsscreen.aucunMessageAvecCeClientPourLe")}</Text> :
          null}

          <View style={styles.infoCard}>
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>{t("screens.owner.reservationdetailsscreen.nomComplet")}</Text>
            ...
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>{t("screens.owner.reservationdetailsscreen.telephone")}</Text>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{t("screens.owner.reservationdetailsscreen.detailDuPrix")}</Text>

          <View style={styles.priceBreakdown}>
            <View style={styles.priceRow}>
              <Text style={styles.priceRowLabel}>
                {totalDays}{t("screens.owner.reservationdetailsscreen.jour")}{totalDays > 1 ? 's' : ''}
              </Text>
              <Text style={styles.priceRowValue}>{formatPrice(rentalSubtotal)}{t("screens.owner.reservationdetailsscreen.da")}</Text>
            </View>

            <View style={styles.dividerSmall} />

            <View style={styles.priceRow}>
              <Text style={styles.priceRowLabel}>{t("screens.owner.reservationdetailsscreen.fraisDeLivraison")}</Text>
              <Text style={styles.priceRowValue}>{formatPrice(deliveryFee)}{t("screens.owner.reservationdetailsscreen.da")}</Text>
            </View>

            <View style={styles.dividerSmall} />

            <View style={[styles.priceRow, styles.totalPriceRow]}>
              <Text style={styles.totalPriceLabel}>{t("screens.owner.reservationdetailsscreen.prixTotal")}</Text>
              <Text style={styles.totalPriceValue}>{formatPrice(safeTotalPrice)}{t("screens.owner.reservationdetailsscreen.da")}</Text>
            </View>
          </View>
          ...
          <View style={styles.escrowStatusCard}>
              <View style={styles.escrowStatusHeader}>
                ...
                <Text style={styles.escrowStatusTitle}>{t("screens.owner.reservationdetailsscreen.escrowCarte")}</Text>
              </View>
          ...
          <View style={styles.cashPaymentAlert}>
              <View style={styles.cashPaymentAlertContent}>
                ...
                <View style={styles.cashPaymentAlertText}>
                  <Text style={styles.cashPaymentAlertTitle}>{t("screens.owner.reservationdetailsscreen.paiementALaRecuperation")}</Text>
                  <Text style={styles.cashPaymentAlertDescription}>
                    {paymentInfo?.status === 'pending_cash' ?
                  'En attente de confirmation du paiement en espèces' : t("screens.owner.reservationdetailsscreen.paiementEnEspecesConfirme")
                  }
                  </Text>
                </View>
              </View>
            ...
            <TouchableOpacity
              ...
              <>
                      ...
                      <Text style={styles.confirmPaymentButtonText}>{t("screens.owner.reservationdetailsscreen.confirmerLePaiement")}</Text>
                    </>
              }
                </TouchableOpacity>
            }
        ...
        <View style={styles.section}>
            <Text style={styles.sectionTitle}>{t("screens.owner.reservationdetailsscreen.recuperation")}</Text>
            <TouchableOpacity
            ...
            style={styles.handoverActionWrap}>
              ...
              style={styles.handoverAction}>
                ...
                <Text style={styles.handoverActionText}>{t("screens.owner.reservationdetailsscreen.verifierLeCodeDeRecuperation")}</Text>
              </LinearGradient>
            </TouchableOpacity>
            <Text style={styles.handoverHint}>{t("screens.owner.reservationdetailsscreen.disponibleUniquementDansLes24hAvantLe")}
        ...
        <View style={styles.section}>
            <Text style={styles.sectionTitle}>{t("screens.owner.reservationdetailsscreen.retour")}</Text>
            <TouchableOpacity
            ...
            style={styles.handoverActionWrap}>
              ...
              style={styles.handoverAction}>
                ...
                <Text style={styles.handoverActionText}>{t("screens.owner.reservationdetailsscreen.afficherLeQrCodeDeRetour")}</Text>
              </LinearGradient>
            </TouchableOpacity>
            <Text style={styles.handoverHint}>{t("screens.owner.reservationdetailsscreen.disponibleUniquementDansLes24hAvantLa")}
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeaderWithButton}>
            <Text style={styles.sectionTitle}>{t("screens.owner.reservationdetailsscreen.statut")}</Text>
            <TouchableOpacity onPress={openStatusPicker} activeOpacity={0.8}>
              <LinearGradient colors={['#a566ff', '#8f6cff']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.statusButton}>
                ...
                <Text style={styles.statusButtonText}>{t("screens.owner.reservationdetailsscreen.mettreAJour")}</Text>
              </LinearGradient>
            </TouchableOpacity>
          </View>
          <View style={styles.infoCard}>
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>{t("screens.owner.reservationdetailsscreen.actuel")}</Text>
      ...
      <View style={styles.loadingOverlay}>
          ...
          <Text style={styles.loadingText}>{t("screens.owner.reservationdetailsscreen.chargement")}</Text>
        </View> :
      null}
      ...
      <Modal transparent visible={statusModalOpen} animationType="fade" onRequestClose={closeStatusPicker}>
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>{t("screens.owner.reservationdetailsscreen.mettreAJourLeStatut")}</Text>
            <Text style={styles.modalSubtitle}>{t("screens.owner.reservationdetailsscreen.choisissezUnStatutPuisConfirmez")}</Text>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity onPress={closeStatusPicker} activeOpacity={0.85} style={styles.modalButtonGhost}>
                <Text style={styles.modalButtonGhostText}>{t("screens.owner.reservationdetailsscreen.annuler")}</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                <Text style={styles.modalButtonPrimaryText}>{t("screens.owner.reservationdetailsscreen.confirmer")}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
